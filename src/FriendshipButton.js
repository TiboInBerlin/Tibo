...
class FriendshipButton extends React.Component {
    ...
    componentDidMount() {
        axios
            .get("/user-friendship/" + this.props.otherUserId + ".json")
            .then(resp => {
                ...
                if (this.state.friendshipStatus == null) {
                    ...
                    this.state.loggedInUserId == this.state.senderId
                ) {
                    console.log(this.state.friendshipStatus);
                    this.setState({
                    ...
                    this.state.friendshipStatus == 1 &&
                    this.state.loggedInUserId == this.state.receiverId
                ) {
                    this.setState({
    ...
    friend() {
        if (this.state.friendshipStatus == null) {
            ...
            axios
                .post("/update-friend", {
                    senderId: this.state.otherUserId,
                    receiverId: this.state.loggedInUserId,
                    status: 2
                })
                .then(results => {
                    this.setState({
